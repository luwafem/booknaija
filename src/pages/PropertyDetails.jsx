// PropertyDetails.jsx — with shortlet "Check Availability" on WhatsApp
...
export default function PropertyDetails() {
  ...
  const handleBookingAction = () => {
    // ─── SHORTLET: Open WhatsApp with availability message ───
    if (isShortlet && biz.whatsapp) {
      const message = encodeURIComponent(
        `Hello, I would like to check availability for: ${property.name} located at ${property.location || 'your listed property'}. Please let me know if it's available for my desired dates.`
      );

      let waNumber = biz.whatsapp.replace(/\D/g, '');
      if (waNumber.startsWith('0')) {
        waNumber = '234' + waNumber.substring(1);
      }
      if (!waNumber.startsWith('234') && waNumber.length === 10) {
        waNumber = '234' + waNumber;
      }

      const base = `https://wa.me/${waNumber}`;
      const separator = base.includes('?') ? '&' : '?';
      const fullUrl = `${base}${separator}text=${message}`;
      window.open(fullUrl, '_blank');
      return;
    }

    // ─── INSPECTION (sale/rent): Open WhatsApp with inspection message ───
    if (isInspection && biz.whatsapp) {
      const message = encodeURIComponent(
    ...
    alert('This property type is not supported for booking.');
  };
            ...
            <div className="lg:col-span-2 flex flex-col">
              ...
              <div className="hidden lg:block mt-auto">
                <button 
                  ...
                  style={!isShortlet && !isInspection ? { backgroundColor: accent } : {}}
                >
                  {isShortlet ? (
                    <>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <button 
            ...
            className={`flex-1 max-w-[200px] flex items-center justify-center gap-2 px-6 py-3.5 text-[10px] font-bold tracking-[0.15em] uppercase transition-all duration-300 rounded-full shadow-sm ${
              isShortlet || isInspection
                ? `border ${isDark ? 'border-white/30 text-white' : 'border-stone-800 text-stone-900'}`
                : 'text-white hover:brightness-110'
            }`}
            style={!isShortlet && !isInspection ? { backgroundColor: accent } : {}}
          >
            {isShortlet ? 'Check Availability' : isInspection ? 'Inspect' : 'Book Now'}
          </button>
        </div>
      </div>
