...
export default function PropertyAbout({ biz, accent, isDark }) {
  ...
  return (
    <section id="about" className={`relative ${theme.bg} transition-colors duration-500 overflow-hidden`}>
              ...
              <div className="border-l-2 pl-8 md:pl-10" style={{ borderColor: accent + '50' }}>
                ...
                <div className="flex flex-wrap gap-2.5 overflow-hidden">
                  {biz.phone && (
                    <a
                      href={`tel:${biz.phone}`}
                      className={`inline-flex items-center gap-2.5 px-4 py-2 text-xs font-medium tracking-wide transition-colors duration-300 ${theme.sub} ${theme.pill} rounded-full hover:opacity-70 break-words`}
                    >
                  ...
                  {biz.location && (
                    <span className={`inline-flex items-center gap-2.5 px-4 py-2 text-xs font-medium tracking-wide ${theme.sub} ${theme.pill} rounded-full break-words`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
              {team.map((member, idx) => (
                <div
                  key={member.id || idx}
                  className={`group text-center p-6 md:p-8 transition-all duration-500 ${isDark ? 'hover:bg-white/[0.03] border border-white/[0.06]' : 'hover:bg-gray-50 border border-gray-100'} rounded-2xl overflow-hidden`}
                >
