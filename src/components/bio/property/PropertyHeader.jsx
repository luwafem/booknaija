...
export default function PropertyHeader({ biz, accent, isDark }) {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-14">
          <div className="flex items-center justify-between h-20">
            ...
            <a href="#" className="flex items-center gap-3.5 group">
              {biz.logo ? (
                <img
                  src={biz.logo}
                  alt={biz.name}
                  className="w-9 h-9 rounded-full object-cover ring-1 ring-white/30 transition-transform duration-300 group-hover:scale-105"
                />
              ) : (
                <div
            ...
            <nav className="hidden md:flex items-center gap-10">
              {['Listings', 'About', 'Contact'].map((item) => (
                <a
                  ...
                  className="relative group transition-colors duration-300 text-white/80 hover:text-white"
                >
                  <span className="text-[11px] font-semibold tracking-[0.2em] uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,0.7)]">
                    {item}
                  </span>
                  <span
                    className="absolute -bottom-1 left-0 w-0 h-[1px] transition-all duration-300 ease-out group-hover:w-full"
            ...
            <div className="flex items-center gap-5">
              {/* Desktop Button - Strictly Outline, NO background */}
              <a
                href={whatsappLink}
                target="_blank"
                rel="noreferrer"
                className="hidden sm:inline-flex items-center gap-2.5 px-6 py-2.5 text-[11px] font-bold tracking-[0.15em] uppercase border border-white/40 text-white hover:border-white/80 transition-all duration-300"
              ...
              <button
                ...
                className="md:hidden w-10 h-10 flex items-center justify-center text-white hover:bg-white/10 rounded-sm transition-colors duration-300"
      ...
      {/* Fullscreen Mobile Menu - RENDERED OUTSIDE HEADER TO FIX MOBILE FIXED BUGS */}
      <div 
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center text-white/70 hover:text-white transition-colors duration-200"
          ...
          >
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-center gap-3 w-full px-6 py-3.5 text-[11px] font-bold tracking-[0.15em] uppercase border border-white/30 text-white hover:border-white/70 transition-all duration-300"
              style={{ backgroundColor: 'transparent' }}
            >
